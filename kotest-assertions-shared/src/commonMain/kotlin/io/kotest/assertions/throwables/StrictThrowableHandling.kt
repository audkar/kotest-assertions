...
inline fun <reified T : Throwable> shouldThrowExactly(block: () -> Any?): T {
   ...
   return when {
      ...
      else -> throw stacktraces.cleanStackTrace(Exceptions.createAssertionError(clueContextAsString() + "Expected exception ${expectedExceptionClass.bestName()} but a ${thrownThrowable::class.simpleName} was thrown instead."))
   }
}
...
inline fun <reified T : Throwable> shouldNotThrowExactly(block: () -> Any?) {
   ...
   if (thrown::class == T::class) throw stacktraces.cleanStackTrace(Exceptions.createAssertionError(clueContextAsString() + "No exception expected, but a ${thrown::class.simpleName} was thrown."))
   throw thrown
}
