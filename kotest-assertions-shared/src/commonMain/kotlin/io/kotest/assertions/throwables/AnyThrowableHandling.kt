...
inline fun <T> shouldNotThrowAny(block: () -> T): T {
   ...
   throw stacktraces.cleanStackTrace(Exceptions.createAssertionError(clueContextAsString() + "No exception expected, but a ${thrownException::class.simpleName} was thrown."))
}
...
inline fun <T> shouldThrowMessage(message: String, block: () -> T) {
   ...
   if (thrownException.message != message) {
      throw stacktraces.cleanStackTrace(Exceptions.createAssertionError(clueContextAsString() + "Expected a throwable with message ${StringShow.show(message).value} but got a throwable with message ${thrownException.message.show().value}".trimMargin()))
   }
}
...
inline fun <T> shouldNotThrowMessage(message: String, block: () -> T) {
   ...
   if (thrownException != null && thrownException.message == message)
      throw stacktraces.cleanStackTrace(Exceptions.createAssertionError(clueContextAsString() + """Expected no exception with message: "$message"
